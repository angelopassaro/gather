...
curl -LO https://github.com/findomain/findomain/releases/latest/download/findomain-linux.zipunzip findomain-linux.zip
chmod +x findomainsudo mv findomain /usr/bin/local/findomain
rm -rf findomainrm findomain-linux.zip

git clone https://github.com/0xKayala/ParamSpider /opt/ParamSpidercd /opt/ParamSpider
sudo python3 -m pip install -r requirements.txtln -s /opt/ParamSpider/paramspider.py /usr/local/bin/paramspider
cd -
sudo git clone https://github.com/maurosoria/dirsearch.git --depth 1 /opt/dirsearch
cd /opt/dirsearchsudo python3 -m pip install -r requirements.txt
...
mv some-PIIs.yaml ~/nuclei-templates/JSA/mv credentials-disclosure-all.yaml ~/nuclei-templates/JSA/